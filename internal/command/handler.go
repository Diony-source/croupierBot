...
// ICommand is an interface that all command structs must implement.
type ICommand interface {
	Name() string
	Execute(s *discordgo.Session, m *discordgo.MessageCreate, args []string)
}

// Handler holds all registered commands.
type Handler struct {
...
// Route takes an incoming message and directs it to the correct command.
